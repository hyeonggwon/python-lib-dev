...
## 4. Amendments가 원본을 이긴다

`docs/discussion-log.md`는 두 블록 구조다:

1. 논의점 1~5 원본 답변 — 설계 스냅샷으로 **보존**.
2. `# Amendments` — 구축 중 바뀐 결정들 (A1~A6).

**충돌 시 Amendments가 정답이다.** 새로운 결정 변경은:
- 원본 섹션을 고치지 말고, Amendments에 `A7. ...` 형태로 **추가**한다.
- `원본 → 실제 + 이유` 형식을 지킨다.
- 필요하면 말미의 "주요 결정 요약 표 갱신"을 함께 업데이트한다.

이유: 의사결정 이력을 보존해야 "왜 지금 이 모양인지"가 재구성 가능하다. 원본을 덮어쓰면 그 맥락이 사라진다.

## 5. 파일별 책임 (어디를 고쳐야 하는가)
...
- 원본 논의점 결정을 discussion-log에서 직접 수정 — 대신 Amendments 추가.
...
- 결정 이력 + Amendments: `docs/discussion-log.md`
