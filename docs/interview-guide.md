...
- `branch_name`:
  - 사용자가 **기본값(`harness/<run-id>`) 으로 OK** 하면 `null` 그대로 둔다 — `run.py` preflight 가 실제 run-id 로 채워준다 (`harness/<actual-run-id>`).
  - 사용자가 **커스텀 이름**(`feat/async-parser` 등) 을 주면 그 문자열을 그대로 저장.
  - new 모드에선 `null`.
  - **금지**: `"harness/<run-id>"` 를 literal 문자열로 저장하지 말 것 — `<run-id>` 가 placeholder 가 아니라 그대로 git 브랜치명에 박힌다.
